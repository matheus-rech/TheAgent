...
import { query, type Options as QueryOptions, type SDKMessage, type SDKAssistantMessage, type SDKResultMessage } from '@anthropic-ai/claude-agent-sdk';
...
/**
 * Structured Extraction Options (Legacy - Direct Anthropic API)
 */
...
/**
 * Agent SDK Extraction Options (New - Agent SDK)
 */
...
/**
 * Perform structured extraction using Agent SDK query pattern
 *
 * @param options - Extraction configuration using Agent SDK
 * @returns Extracted structured data from tool input
 *
 * @example
 * ```typescript
 ...
 * ```
 */
export async function performStructuredExtractionWithAgent<T = any>(
  ...
  if (verbose) {
    console.log(`[AgentExtraction] Using tool: ${tool.name}`);
    console.log(`[AgentExtraction] Model: ${model}`);
  }
  ...
  // Prepare system prompt with tool instruction
  const systemPrompt = `You are a precise data extraction assistant. Use the ${tool.name} tool to extract structured data according to its schema. Extract only explicitly stated information and use null for missing fields.`;
  ...
  if (verbose) {
    console.log(`[AgentExtraction] Starting Agent SDK query...`);
  }
  ...
  for await (const message of queryGen) {
    ...
    if (verbose) {
      console.log(`[AgentExtraction] Received message type: ${message.type}`);
    }

    // Look for assistant message with tool use
    if (message.type === 'assistant') {
      lastAssistantMessage = (message as SDKAssistantMessage).message;
      ...
      for (const block of lastAssistantMessage.content || []) {
        if (block.type === 'tool_use' && block.name === tool.name) {
          extractedData = block.input;
          if (verbose) {
            console.log(`[AgentExtraction] Extracted data from tool_use block`);
          }
        } else if ((block as any).type === 'citations') {
          citations.push(...(block as any).citations);
          if (verbose) {
            console.log(`[AgentExtraction] Collected ${(block as any).citations.length} citations`);
          }
        }
      }
    }
    ...
    if (message.type === 'result') {
      ...
      if (resultMsg.subtype === 'success' && resultMsg.structured_output) {
        ...
        extractedData = resultMsg.structured_output;
        if (verbose) {
          console.log(`[AgentExtraction] Using structured_output from result message`);
        }
      }
    }
  }
  ...
  if (!extractedData) {
    throw new Error(`No tool_use block found with name "${tool.name}" in Agent SDK response`);
  }

  // REQUIRED: Create displaySources function for transparency
  const displaySources = enableCitations && citations.length > 0 && lastAssistantMessage
    ? () => {
      const sources = extractCitationSources(lastAssistantMessage);
      displayCitationSources(sources, 'Extraction Sources');
    }
    : undefined;
...
/**
 * Extract methods section using structured tool pattern (Legacy - Direct API)
 */
...
/**
 * Extract methods section using Agent SDK
 */
export async function extractMethodsSectionWithAgent(
  methodsText: string,
  options?: { verbose?: boolean; agentOptions?: Partial<QueryOptions> }
): Promise<any> {
  const result = await performStructuredExtractionWithAgent({
    documentContent: methodsText,
    prompt: 'Extract all methodological details from this Methods section text.',
...
/**
 * Extract results section using structured tool pattern (Legacy - Direct API)
 */
...
/**
 * Extract results section using Agent SDK
 */
export async function extractResultsSectionWithAgent(
  resultsText: string,
  options?: { verbose?: boolean; agentOptions?: Partial<QueryOptions> }
): Promise<any> {
  const result = await performStructuredExtractionWithAgent({
...
/**
 * Extract citations using structured tool pattern (Legacy - Direct API)
 */
...
/**
 * Extract citations using Agent SDK
 */
export async function extractCitationsStructuredWithAgent(
  referencesText: string,
  options?: { verbose?: boolean; agentOptions?: Partial<QueryOptions> }
): Promise<any[]> {
  const result = await performStructuredExtractionWithAgent({
